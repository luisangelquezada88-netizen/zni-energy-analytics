...
st.title("Generación eléctrica de las Zonas No Interconectadas")
...
c3.metric("Departamentos", f"{sel['departamento'].nunique()}")
c4.metric("Diésel en matriz", "78,2 %")
...
with col_scatter:
    st.subheader("Grupos de riesgo (modelo)")
    puntos = df_mpio[df_mpio["riesgo"].isin(riesgos)].reset_index()
...
# Fila 4: perfiles + descarga
st.subheader("Perfiles de riesgo")
perfiles = df_mpio[df_mpio["riesgo"].isin(riesgos)].groupby("riesgo").agg(
    municipios=("horas_media", "count"), horas_media=("horas_media", "mean"),
    energia_media=("energia_media", "mean")).round(2).sort_values("horas_media")
